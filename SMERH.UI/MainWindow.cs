...
using System.Linq; // Удобные запросы к данным как на sql и фильтрация tcp/udp соединений
...
using SMERH.MainCore; // Библиотека с основными методами
...
namespace SMERH // Пространство имен служащее для логической группировки связанных классов всего приложения в данном случае
{
    public partial class MainWindow_SMA : MetroForm // Объявление класса, окно приложения
    {
        ...
        private async void ShowProcessTree()
        {
            List<(string name, string pid, string parent_pid)> listOfProcesses = await MainCoreService.getProcessesTree("" + _trackedProcess.Id, 300);

            foreach (var process in listOfProcesses) {
                AppendOutputSafe($"PID={process.pid},Name={process.name},ParentPID={process.parent_pid}");
            }
        }
